...
| tmux | 内容 | 产出 |
|---|---|---|
| (当前) | `run_multiseed` geom×5种子 SSE+CSE | runs/multiseed/multiseed.md |
> 历史已完成：finalist(final_m3/final_m16)、CSE全量(runs/cse)、Waymo SSE geom(runs/waymo_sse_geom)、CARLA→Waymo迁移(runs/waymo_transfer)、encoder消融最终配方(runs/encoder_ablation_final)、敏感性(runs/sup_sens)、Waymo诊断、CARLA基线SSE(runs/sse)。
...
| **Waymo SSE(in-domain)** | 30.16 | 30.22(+0.06) | 基线已高、空间小；LPIPS 0.243→0.235 |
| **CARLA→Waymo零样本** | 30.16 | **26.63(−3.5)** | ⚠️**负迁移**，sim2real gap |
...
- **(B)真实数据**：Waymo in-domain≈基线(基线已高)、LPIPS改善；**零样本负迁移→需in-domain+讨论domain gap**。诚实呈现。
